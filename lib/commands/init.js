...
async function init({ cwd, flags }) {
  ...
  // Roles
  for (const r of ["generator", "integrator", "tester", "debugger", "documenter", "security"]) {
    plan.push({ rel: `.agent/roles/${r}.yaml`, content: T.roleYaml(r, persona, detection), mode: "create-only" });
  }
  ...
  if (flags.learn) {
    ...
    fs.writeFileSync(draftPath, T.conventionsDraftYaml(inferences));
    process.stdout.write(`\n      draft saved → .agent/conventions.draft.yaml\n`);
    process.stdout.write(`      review it, then merge into .agent/conventions.yaml\n`);
  }
