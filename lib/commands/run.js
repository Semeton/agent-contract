"use strict";

const fs = require("fs");
const os = require("os");
const path = require("path");
const { execFileSync } = require("child_process");
const { loadConfig } = require("../util/config");
const { getProvider, detectProvider } = require("../providers");
const { estimateTokens, checkUsage, WARN_THRESHOLD, HANDOFF_THRESHOLD, PREFLIGHT_THRESHOLD } = require("../util/tokens");

const BOX_WIDTH = 62;
const T = require("../util/templates");

async function run({ cwd, flags }) {
  const role = flags.role || (flags._ && flags._[0]);
  const task = flags.task || (flags._ && flags._.slice(1).join(" "));
  const specFile = flags.spec || null;
  const dryRun = !!flags.dryRun;

  if (!role) {
    process.stderr.write("error: --role <name> is required\n");
    return 1;
  }
  if (!task && !specFile) {
    process.stderr.write("error: --task <description> or --spec <file> is required\n");
    return 1;
  }

  // 1. Validate contract presence
  const manifestPath = path.join(cwd, ".agent/manifest.yaml");
  if (!fs.existsSync(manifestPath)) {
    process.stderr.write(`error: no .agent/manifest.yaml in ${cwd} — run \`agent-contract init\` first\n`);
    return 1;
  }

  const rolePath = path.join(cwd, `.agent/roles/${role}.yaml`);
  if (!fs.existsSync(rolePath)) {
    process.stderr.write(`error: role not found: ${rolePath}\n`);
    return 1;
  }

  // 2. Resolve provider + model
  const config = loadConfig(cwd);
  const providerName = dryRun ? "echo" : (flags.provider || config.provider || detectProvider());
  const model = config.model || undefined;

  process.stdout.write("agent-contract run\n");
  process.stdout.write(`  target:   ${cwd}\n`);
  process.stdout.write(`  role:     ${role}\n`);
  process.stdout.write(`  provider: ${providerName}\n`);
  if (dryRun) process.stdout.write("  mode:     dry-run\n");
  process.stdout.write("\n");

  // 3. Load contract files
  process.stdout.write("[1/4] loading contract…\n");
  const roleContent = fs.readFileSync(rolePath, "utf8");
  const stackPath = path.join(cwd, ".agent/stack.yaml");
  const conventionsPath = path.join(cwd, ".agent/conventions.yaml");
  const stackContent = fs.existsSync(stackPath) ? fs.readFileSync(stackPath, "utf8") : "";
  const conventionsContent = fs.existsSync(conventionsPath) ? fs.readFileSync(conventionsPath, "utf8") : "";
  process.stdout.write(`      manifest + stack + conventions + ${role} role loaded\n\n`);

  // 4. Build or load task spec
  let spec;
  if (specFile) {
    if (!fs.existsSync(specFile)) {
      process.stderr.write(`error: spec file not found: ${specFile}\n`);
      return 1;
    }
    try {
      spec = JSON.parse(fs.readFileSync(specFile, "utf8"));
    } catch (e) {
      process.stderr.write(`error: invalid JSON in spec file: ${e.message}\n`);
      return 1;
    }
  } else {
    spec = {
      feature_name: task,
      input_shape: "unspecified",
      output_shape: "unspecified",
      error_cases: [],
    };
  }

  // 5. Pre-generate check — validates required inputs for any role.
  const preCheckPath = path.join(cwd, ".agent/checks/pre-generate.sh");
  let tmpSpec = null;
  if (fs.existsSync(preCheckPath)) {
    process.stdout.write("[2/4] running pre-generate check…\n");
    tmpSpec = path.join(os.tmpdir(), `agent-spec-${Date.now()}.json`);
    fs.writeFileSync(tmpSpec, JSON.stringify(spec));
    const checkExit = runScript(preCheckPath, [tmpSpec, role]);
    if (checkExit !== 0) {
      cleanup(tmpSpec);
      process.stderr.write("error: pre-generate check failed — task spec is incomplete\n");
      return 1;
    }
    process.stdout.write("      pre-generate: OK\n\n");
  } else {
    process.stdout.write("[2/4] pre-generate check: skipped (not found)\n\n");
  }

  // 6. Assemble prompt
  process.stdout.write("[3/4] calling provider…\n");
  const { system, user } = assemblePrompt({ role, roleContent, stackContent, conventionsContent, spec });

  // Pre-flight: warn if estimated prompt alone is already large
  const estimatedPromptTokens = estimateTokens(system + user);
  const estimatedModel = model || (providerName === "openai" ? "gpt-4o" : "claude-opus-4-8");
  const preflightCheck = checkUsage({ input_tokens: estimatedPromptTokens, output_tokens: 0 }, estimatedModel);
  if (preflightCheck.ratio >= PREFLIGHT_THRESHOLD) {
    process.stdout.write(
      `      ⚠  prompt is ~${preflightCheck.pct}% of the ${estimatedModel} context window before the model has even responded.\n` +
      `         Consider breaking this task into smaller pieces.\n`
    );
  }

  // 7. Call provider
  let result;
  try {
    const provider = getProvider(providerName);
    result = await provider.generate({ system, user, model });
  } catch (err) {
    cleanup(tmpSpec);
    process.stderr.write(`error: provider call failed — ${err.message}\n`);
    return 1;
  } finally {
    cleanup(tmpSpec);
  }
  process.stdout.write("\n");

  // Token usage assessment
  const usage = result.usage || { input_tokens: estimatedPromptTokens, output_tokens: estimateTokens(result.output) };
  const usedModel = result.model || estimatedModel;
  const tokenInfo = checkUsage(usage, usedModel);
  process.stdout.write(`      tokens: ${tokenInfo.total.toLocaleString()} / ${tokenInfo.limit.toLocaleString()} (${tokenInfo.pct}%)\n`);

  if (tokenInfo.ratio >= HANDOFF_THRESHOLD) {
    // Generate handoff note
    const recentDecisions = loadRecentDecisions(path.join(cwd, ".agent/memory/decisions.jsonl"));
    const ts = Date.now();
    const handoffPath = path.join(cwd, `.agent/memory/handoff-${ts}.md`);
    const noteContent = T.handoffNote({
      role,
      task: spec.feature_name,
      provider: providerName,
      model: usedModel,
      usage: tokenInfo,
      recentDecisions,
    });
    let handoffSaved = true;
    try {
      fs.mkdirSync(path.dirname(handoffPath), { recursive: true });
      fs.writeFileSync(handoffPath, noteContent);
    } catch (e) {
      handoffSaved = false;
      process.stderr.write(`  ⚠  handoff write failed: ${e.message}\n`);
    }

    process.stdout.write("\n");
    process.stdout.write("  ╔══════════════════════════════════════════════════════════╗\n");
    process.stdout.write(`  ║  CONTEXT WINDOW AT ${String(tokenInfo.pct + "%").padEnd(4)} — HANDOFF GENERATED              ║\n`);
    process.stdout.write("  ╠══════════════════════════════════════════════════════════╣\n");
    process.stdout.write(`  ║  ${handoffSaved ? "Saved" : "FAILED"}: .agent/memory/handoff-${ts}.md`.padEnd(BOX_WIDTH - 1) + "║\n");
    process.stdout.write("  ║                                                          ║\n");
    process.stdout.write("  ║  Start a fresh session and open that file for context.   ║\n");
    process.stdout.write("  ╚══════════════════════════════════════════════════════════╝\n");
    process.stdout.write("\n");
  } else if (tokenInfo.ratio >= WARN_THRESHOLD) {
    process.stdout.write(
      `      ⚠  context window at ${tokenInfo.pct}% — consider starting a fresh session soon.\n`
    );
  }

  // 8. Post-generate check
  const postCheckPath = path.join(cwd, ".agent/checks/post-generate.sh");
  if (fs.existsSync(postCheckPath)) {
    process.stdout.write("[4/4] running post-generate check…\n");
    const checkExit = runScript(postCheckPath, []);
    if (checkExit !== 0) {
      process.stderr.write("error: post-generate check failed — scope or size violation\n");
      return 1;
    }
    process.stdout.write("      post-generate: OK\n\n");
  }

  // 9. Append to decisions.jsonl
  const decisionsPath = path.join(cwd, ".agent/memory/decisions.jsonl");
  if (fs.existsSync(path.dirname(decisionsPath))) {
    const entry = {
      ts: new Date().toISOString(),
      role,
      task: spec.feature_name,
      provider: providerName,
      model: usedModel,
      checks: {
        pre_generate: "pass",
        post_generate: "pass",
      },
      output_lines: result.output.split("\n").length,
      tokens: { used: tokenInfo.total, limit: tokenInfo.limit, pct: tokenInfo.pct },
    };
    fs.appendFileSync(decisionsPath, JSON.stringify(entry) + "\n");

    // Compact decisions.jsonl — keep the last 200 entries, archive the rest.
    try {
      const raw = fs.readFileSync(decisionsPath, "utf8").trim();
      const lines = raw ? raw.split("\n") : [];
      if (lines.length > 200) {
        const archivePath = path.join(cwd, ".agent/memory/decisions-archive.jsonl");
        fs.appendFileSync(archivePath, lines.slice(0, -200).join("\n") + "\n");
        fs.writeFileSync(decisionsPath, lines.slice(-200).join("\n") + "\n");
      }
    } catch { /* best-effort */ }
  }

  // 10. Print output
  process.stdout.write(result.output + "\n");

  // 11. Task-completion handoff — written on every successful run so context is never silently lost.
  //     If the threshold handoff already fired (step 7), it emitted the banner; this note is still
  //     written but we suppress the duplicate "start a fresh session" message.
  const completionTs = Date.now();
  const completionHandoffPath = path.join(cwd, `.agent/memory/handoff-done-${completionTs}.md`);
  const completionNote = T.handoffNote({
    role,
    task: spec.feature_name,
    provider: providerName,
    model: usedModel,
    usage: tokenInfo,
    recentDecisions: loadRecentDecisions(decisionsPath),
    reason: "completion",
  });
  try {
    fs.mkdirSync(path.dirname(completionHandoffPath), { recursive: true });
    fs.writeFileSync(completionHandoffPath, completionNote);
  } catch {}

  if (tokenInfo.ratio < HANDOFF_THRESHOLD) {
    process.stdout.write(`  ✓ task complete — handoff saved → .agent/memory/handoff-done-${completionTs}.md\n`);
    process.stdout.write(`    Recommend: start a fresh session for your next task.\n\n`);
  }

  return 0;
}

function assemblePrompt({ role, roleContent, stackContent, conventionsContent, spec }) {
  const system = [
    "You are operating under an agent contract. Follow all role instructions exactly.",
    "Do not take actions outside the scope defined in your role. When inputs are missing or ambiguous, ask the human.",
    "",
    "## Stack",
    stackContent || "(no stack detected)",
    "",
    "## Engineering Conventions",
    conventionsContent || "(no conventions defined)",
    "",
    `## Your Role: ${role}`,
    roleContent,
  ].join("\n");

  const userLines = ["## Task Specification"];
  for (const [k, v] of Object.entries(spec)) {
    const val = Array.isArray(v) ? (v.length ? v.join(", ") : "none") : String(v);
    userLines.push(`${k}: ${val}`);
  }
  userLines.push("", "Proceed according to your role. If any field is \"unspecified\", ask the human before continuing.");

  return { system, user: userLines.join("\n") };
}

function runScript(scriptPath, args) {
  try {
    execFileSync("bash", [scriptPath, ...args], { stdio: "pipe" });
    return 0;
  } catch (e) {
    if (e.stderr && e.stderr.length) process.stderr.write(e.stderr);
    return e.status || 1;
  }
}

function cleanup(tmpPath) {
  if (tmpPath) {
    try { fs.unlinkSync(tmpPath); } catch {}
  }
}

function loadRecentDecisions(decisionsPath) {
  try {
    const lines = fs.readFileSync(decisionsPath, "utf8").trim().split("\n").filter(Boolean);
    return lines.map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
  } catch {
    return [];
  }
}

module.exports = { run };
